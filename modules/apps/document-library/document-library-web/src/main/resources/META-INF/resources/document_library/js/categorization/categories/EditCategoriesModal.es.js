...
const EditCategoriesModal = ({
	...
	const [requiredVocabularies, setRequiredVocabularies] = useState([]);
	...
	const getFinalCategories = () => {
		return vocabularies.reduce((accumulator, vocabulary) => {
			vocabulary.selectedItems.map(({value}) => {
				if (!accumulator.includes(value)) {
					accumulator.push(parseInt(value, 10));
				}
			});
			return accumulator;
		}, []);
	...
	const handleVocabulariesChange = newVocabularies => {
		...
		setRequiredVocabularies(requiredVocabularies);
