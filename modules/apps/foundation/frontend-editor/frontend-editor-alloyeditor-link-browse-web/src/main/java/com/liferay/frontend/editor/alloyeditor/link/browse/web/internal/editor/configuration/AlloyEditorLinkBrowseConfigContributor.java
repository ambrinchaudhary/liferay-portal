...
public class AlloyEditorLinkBrowseConfigContributor
	...
	protected void createButtonJSONObject(
		JSONArray buttonsJSONArray, String buttonName) {
		...
		for (int i = 0; i < oldButtonsJSONArray.length(); i++) {
			...
			if (oldButtonJSONObject == null) {
				...
				if (buttonName.equals("link")) {
					createButtonJSONObject(newButtonsJSONArray, "linkBrowse");
				}
				else if (buttonName.equals("linkEdit")) {
					createButtonJSONObject(
						newButtonsJSONArray, "linkEditBrowse");
				}
				else {
